...
class MyPortfolio(BasePortfolio):
    def my_portfolio(self, transactions_df: pd.DataFrame, tickers_prices: pd.DataFrame):
        # Tickers
        tickers_invested_amounts = self.tickers_investment_amount_evolution(transactions_df)
        tickers_pru = self.calculate_pru(transactions_df, tickers_invested_amounts)
        tickers_valuation, tickers_gain_pct, tickers_gain = self.capital_gain_losses_composed(tickers_invested_amounts, tickers_pru, tickers_prices)

        tickers_twr = tickers_gain_pct
        tickers_gain = tickers_gain
        tickers_valuation = tickers_valuation
        tickers_dividends = self.calculate_dividends(transactions_df)
    ...
    def calculate_dividends(self, transactions_df: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        dividends_df = transactions_df[transactions_df["operation"] == "dividend"].copy()
